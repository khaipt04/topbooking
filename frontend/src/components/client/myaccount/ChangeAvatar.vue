<script setup>
import ChangeAvatarForm from "@/components/client/myaccount/ChangeAvatarForm.vue";
import {
  Dialog,
  DialogPanel,
  TransitionChild,
          ...
          <TransitionChild as="template" enter="ease-out duration-300" enter-from="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95" enter-to="opacity-100 translate-y-0 sm:scale-100" leave="ease-in duration-200" leave-from="opacity-100 translate-y-0 sm:scale-100" leave-to="opacity-0 translate-y-4 sm:translate-y-0 sm:scale-95">
            <DialogPanel class="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all w-9/10 sm:w-80 lg:w-100 p-3">
              <div class="flex justify-between items-center">
                ...
                <XMarkIcon @click="$emit('update:modelValue', false); $emit('open')" class="h-6 w-6 text-gray-500 cursor-pointer" />
              </div>
              <div class="mt-10">
